---
locale: it
page:
  heroEyebrow: Percorso
  heroTitle: Interpretazioni
  heroDescription: Personaggi operistici portati in scena da Sol Risé — con storico delle esibizioni, sedi e fotografie di ogni produzione.
items:
  annina-la-traviata:
    summary: La fedele cameriera di Violetta — interpretata a Buenos Aires.
    tags:
      - Verdi
      - Mezzosoprano
    content: Annina è un ruolo di carattere che richiede arguzia e precisione d'ensemble nel *La Traviata* di Verdi. Sol Risé ha interpretato il ruolo a Buenos Aires.
  apparitions-macbeth:
    characterName: Apparizione
    summary: Scene di coro e apparizioni nella tragedia shakespeariana di Verdi — con Juventus Lyrica a Buenos Aires.
    tags:
      - Verdi
      - Coro
    content: Sol Risé ha partecipato al coro e alle sequenze di apparizione del *Macbeth* di Verdi con Juventus Lyrica — musica che richiede peso drammatico e precisione d'ensemble.
    appearances:
      - notes: Membro del coro e scene di apparizione
  contessa-di-ceprano-rigoletto:
    summary: Una dama di corte nella tragedia di Verdi — interpretata con Celebrarte Música a Buenos Aires.
    content: La Contessa di Ceprano compare nella scena d'apertura di corte de *Rigoletto* di Verdi. Sol Risé ha interpretato il ruolo con Celebrarte Música.
  chorus-carmen:
    summary: Coro e rinforzo del coro nell'opera di Bizet — con Juventus Lyrica e Celebrarte Música a Buenos Aires.
    tags:
      - Bizet
      - Coro
    content: Sol Risé ha partecipato al coro de *Carmen* di Bizet con Juventus Lyrica al Teatro Avenida e come rinforzo del coro con Celebrarte Música.
    appearances:
      - notes: Membro del coro
      - notes: Rinforzo del coro
  chorus-cavalleria-rusticana:
    characterName: Coro e solista in evidenza
    summary: Coro in *Cavalleria Rusticana* e solista in evidenza nelle canzonette italiane al Festival della Canzonetta Italiana — con Juventus Lyrica.
    tags:
      - Mascagni
      - Coro
    content: Sol Risé ha cantato nel coro de *Cavalleria Rusticana* di Mascagni ed è apparsa come solista in evidenza nelle canzonette italiane al Festival della Canzonetta Italiana con Juventus Lyrica.
    appearances:
      - notes: Membro del coro e solista in evidenza nelle canzonette italiane
  chorus-il-trovatore:
    summary: Membro del coro nel dramma appassionato di Verdi — con Valkyria Lírica a Buenos Aires.
    tags:
      - Verdi
      - Coro
    content: Sol Risé ha cantato nel coro de *Il Trovatore* di Verdi con Valkyria Lírica — musica che richiede intensità drammatica e precisione d'ensemble.
    appearances:
      - notes: Membro del coro
  first-lady-die-zauberflote:
    characterName: Prima Dama
    summary: Una delle dame della Regina della Notte — interpretata in produzioni in spagnolo per bambini con Celebrarte Música e Jóvenes Talentos.
    tags:
      - Mozart
      - Ruolo di carattere
    content: La Prima Dama (*Erste Dame*) compare nelle scene iniziali de *Die Zauberflöte* di Mozart. Sol Risé ha interpretato il ruolo in produzioni in spagnolo rivolte alle famiglie a Buenos Aires.
    appearances:
      - notes: Adattamento in spagnolo per bambini
      - notes: Adattamento in spagnolo per bambini
  madama-butterfly-aunt:
    characterName: Zia di Cio-Cio-San
    summary: Lavoro d'ensemble e di coro nella tragedia di Puccini — con Juventus Lyrica al Teatro Avenida.
    content: Sol Risé è apparsa come Zia di Cio-Cio-San e nel coro de *Madama Butterfly* di Puccini con Juventus Lyrica.
    appearances:
      - notes: Zia di Madama Butterfly; membro del coro
  mrs-gobineau-the-medium:
    characterName: Signora Gobineau
    summary: Una delle ospiti della seduta spiritica nell'opera da camera di Menotti — con Orquesta Jóvenes Talentos.
    tags:
      - Menotti
      - Ruolo di carattere
    content: La Signora Gobineau fa parte dello stretto ensemble al centro de *The Medium* di Menotti. Sol Risé ha interpretato il ruolo con Orquesta Jóvenes Talentos a Buenos Aires.
  pamina-die-zauberflote:
    summary: La nobile eroina di Mozart — interpretata in una produzione ridotta in tedesco per giovani spettatori al British Arts Centre.
    tags:
      - Mozart
      - Soprano protagonista
    content: Pamina richiede calore vocale e sincerità drammatica. Sol Risé ha affrontato il ruolo in allestimenti scenici e in concerto, tra cui *Die Zauberflöte* di Mozart in un adattamento in tedesco per giovani spettatori.
    appearances:
      - notes: Versione ridotta in tedesco per bambini
      - notes: Esecuzione in concerto
  sister-genovieffa-suor-angelica:
    characterName: Suor Genovieffa
    summary: Una delle suore nella tragedia in un atto di Puccini — interpretata con Celebrarte Música e prossimamente a La Redonda de Belgrano.
    content: Suor Genovieffa compare nell'ensemble d'apertura de *Suor Angelica* di Puccini — un ruolo che si inserisce nel dramma intimo del convento e richiede precisione nel canto d'insieme.
    appearances:
      - notes: Prossima esecuzione
  suora-cercatrice-suor-angelica:
    characterName: Sorella Cercatrice
    summary: La Sorella Cercatrice nel dramma conventuale di Puccini — interpretata con Celebrarte Música.
    content: La Sorella Cercatrice fa parte della comunità al centro de *Suor Angelica* di Puccini. Sol Risé ha interpretato il ruolo con Celebrarte Música a Buenos Aires.
  zerlina-don-giovanni:
    summary: Una sposa contadina vivace, tra fedeltà e seduzione — Sol Risé ha debuttato in questo ruolo con Juventus Lyrica al Teatro Avenida nel 2023.
    content: Sol Risé ha interpretato Zerlina nel *Don Giovanni* di Mozart con Juventus Lyrica al Teatro Avenida — un ruolo che unisce fascino lirico e tensione drammatica nella scena nuziale e nella celebre aria *Batti, batti, o bel Masetto*.
    appearances:
      - notes: Produzione Juventus Lyrica
---

IT role translations (overlay on canonical content/roles/*.md).
