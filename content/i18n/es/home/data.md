---
type: HomePage
headerBrandLine1: Sol Risé
headerBrandLine2: Soprano
headerNavLinks:
  - label: Home
    href: /
  - label: Bio
    href: /bio
  - label: Career
    href: /career
  - label: Schedule
    href: /schedule
  - label: Gallery
    href: /gallery
  - label: Contact
    href: /contact
heroTitle: Sol Risé
heroSubtitle: Soprano
heroTagline: ''
heroImage: /images/don-giovanni/IMG_1371.jpg
heroImageAlt: Sol Risé como Zerlina en Don Giovanni en el Teatro Avenida
primaryCtaLabel: ''
primaryCtaHref: ''
secondaryCtaLabel: ''
secondaryCtaHref: ''
aboutSurface: soft
aboutEyebrow: Sobre
aboutTitle: Un camino lírico en evolución
aboutText: |
  Sol Risé es una soprano argentina con una carrera desarrollada en ópera, repertorio de concierto y canto coral. Su trabajo incluye roles como Pamina (*La flauta mágica*) y Zerlina (*Don Giovanni*), además de participaciones corales en producciones de *La Traviata*, *Carmen* y *Macbeth*.

  Ha actuado con compañías y festivales consolidados en Argentina y Bolivia, combinando experiencia escénica con formación vocal e interpretativa continua. Su trayectoria refleja constancia, versatilidad y un fuerte foco en la expresión musical.
aboutImage: /images/plaza-belgica/IMG_9816.jpeg
aboutImageAlt: Retrato de Sol Risé en Plaza Bélgica, Buenos Aires
aboutLinkText: Ver trayectoria
aboutHref: /career
organizationsStripEyebrow: Colaboraciones
organizationsStripTitle: Entre figuras destacadas
organizationsStripDescription: Teatros y compañías donde Sol Risé ha actuado y continúa desarrollando su repertorio.
organizationsStripLinkText: Ver todas las instituciones
organizationsStripItems:
  - teatro-colon
  - teatro-avenida
  - juventus-lyrica
  - british-arts-center
organizationsStripColorScheme: bright
mediaEyebrow: En escena y en pantalla
mediaTitle: Últimas presentaciones
mediaLinkText: Ver todas las apariciones
featuredEventsLayout: scheduleCards
splitGridItems:
  - title: Suor Angelica
    href: /schedule/suor-angelica-2026
    image: images/schedule-images/suor-angelica.jpeg
    badges:
      - 28 de junio
    subtitle: Buenos Aires - La Redonda de Belgrano
splitGridColorScheme: wine
splitGridTitle: Próximos eventos
splitGridDescription: Dónde ver a Sol Risé — fechas, sedes y detalles de cada producción.
quoteText: La musica è la lingua dell'anima
quoteImage: /images/general/liana-unsplash.webp
quoteImageAlt: Primer plano de una partitura con fondo difuminado
quoteImageCredit:
  author: Liana S
  authorUrl: https://unsplash.com/es/@cherstve_pechivo?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText
  sourceUrl: https://unsplash.com/es/fotos/primer-plano-de-partitura-con-fondo-difuminado-BoG8tpchiis?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText
footerBrandLine1: Sol Risé
footerBrandLine2: Soprano
footerBrandTagline: Soprano · Artista escénica
footerNavLinks:
  - label: Home
    href: /
  - label: Bio
    href: /bio
  - label: Career
    href: /career
  - label: Schedule
    href: /schedule
  - label: Gallery
    href: /gallery
  - label: Contact
    href: /contact
instagramUrl: https://instagram.com/solrisesoprano/
youtubeUrl: https://www.youtube.com/channel/UCgm68FC8sM_2r3cAXBQdbvw
facebookUrl: https://facebook.com/solrisesoprano
muvacUrl: https://www.muvac.com/profile/florencia-sol-rise-lopez
email: solrisesoprano@gmail.com
heroColorScheme: wine
mediaGridColorScheme: soft
quoteBannerColorScheme: wine
---
