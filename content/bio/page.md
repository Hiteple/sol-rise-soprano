---
type: BioPage
pageHeroColorScheme: bright
heroEyebrow: Bio
heroTitle: Artistic Path
heroDescription: ''
fullBioEyebrow: Background & Training
fullBioParagraphs:
  - content: >-
      Sol Risé discovered her voice in Buenos Aires at a young age, and has
      since built a career across opera productions, recitals, and concert
      stages.
    addBorderBottom: false
  - content: >-
      Her work balances vocal technique, dramatic interpretation, and a deep
      commitment to storytelling in every performance.
    addBorderBottom: true
  - content: >+
      Her current formal training includes:


      *   Advanced studies in lyric singing at the [**Instituto Superior de Arte
      del Teatro Colón**](https://teatrocolon.org.ar/instituto-superior-de-arte/),
      where she is currently completing the final year of the program.


      *   Developing the roles of 'Violetta Valéry' (*La Traviata*) and 'Adina'
      (*L'elisir d'amore*) at [**Vincerò Academy**](https://vinceroacademy.com/).

    addBorderBottom: true
  - content: >+
      She has expanded her technique through:


      *   Masterclasses with mezzo-soprano **Daniela Barcelona** (part of the ISATC
      program).


      *   Online masterclasses with **Lisette Oropesa**, **Jennifer Rowley**,
      and **Michael Fabiano.**


      *   Scholarship programs with [**Juventus Lyrica**](https://juventuslyrica.ar/).


      *   Vocal technique studies with **Gustavo López Manzitti** and **Alejandra
      Malvino**, the "La Voz en Movimiento" seminar (*Sol Lírica*).


      *   Musical-theater training at **Fundación Julio Bocca** and the **Pepe
      Cibrián** workshop.

    addBorderBottom: true
  - content: >-
      Her languages and diction include native Spanish, advanced English, and
      operatic phonetics in Italian, German, and French.
    addBorderBottom: false
fullBioColorScheme: soft
fullBioImage: '/images/photo-book/SON05945 1.jpg'
fullBioImageAlt: Sol Risé — Photo Book portrait, hands cupping face in navy sequined top
fullBioImagePhotography: Diego Israelit
fullBioImagePosition: right
---

Biography page body (optional).
